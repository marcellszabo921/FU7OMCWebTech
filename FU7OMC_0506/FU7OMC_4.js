$(document).ready(function(){
    ...
    $.getJSON('FU7OMC_orarend.json', function(data) {
        var kurzusok = [];
        // Kurzusok tömb feltöltése
        for (var i = 0; i < data["SZM orarend"].kurzus.length; i++) {
          var kurzus = data["SZM orarend"].kurzus[i];
    ...
    $('#megjelenit').click(function() {
        ...
        $.getJSON('FU7OMC_orarend.json', function(data) {
            ...
            let elsoKurzus = data["SZM orarend"].kurzus[0].kurzusnev;
            $('#orarend').append('<p><b>Kurzusnév:</b> ' + elsoKurzus + '</p>');
     
            let elsoOktato = data["SZM orarend"].kurzus[0].oktato;
            $('#orarend').append("<p><b>Okatató: </b>"+ elsoOktato+"</p>");

            let szak = data["SZM orarend"].kurzus[0].szak;
            $('#orarend').append("<p><b>Szak: </b>"+szak+"</p>")

            elsoKurzus = data["SZM orarend"].kurzus[0];
            ...
            let helyszin = data["SZM orarend"].kurzus[0].helyszin ;
